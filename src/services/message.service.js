...
class MessageService extends BaseService {
	async sendMessage(room_id, data, token, wss) {
		try {
			...
			this.emitUpdateRoomMessage(wss, room_id, usersRoom, decode, newMessage, 'sendMessage');
	...
	async deleteMessage(message_id, token, wss) {
		try {
			...
			this.emitUpdateRoomMessage(wss, room[0].id, usersRoom, decode, deletedMessage, 'deleteMessageInRoom');
	...
	async updateMessage(message_id, token, data, wss) {
		try {
			...
			this.emitUpdateRoomMessage(wss, room[0].id, usersRoom, decode, updateMessage, 'updateMessageInRoom');
